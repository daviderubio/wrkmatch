...
from wrkmatch import read_connections, discover_and_fetch, compute_scores, normalize_company_name
...
with st.container():
    ...
    comp_counts = (connections_df.assign(norm=lambda d: d["Company"].map(normalize_company_name))
...
if not jobs_df.empty:
    st.success(f"Found {len(jobs_df):,} job postings across {jobs_df['company'].nunique():,} of your companies.")
    ...
    with st.container():
        ...
        src_counts = jobs_df.groupby("source").size().reset_index(name="count").sort_values("count", ascending=False)
        ...
        chart_src = alt.Chart(src_counts).mark_bar().encode(
        ...
        if jobs_df["posted_at_dt"].notna().any():
            ts = (jobs_df.dropna(subset=["posted_at_dt"])
    ...
    score_df = compute_scores(connections_df, jobs_df)
    ...
    filtered = score_df[(score_df["contacts"] >= min_contacts) & (score_df["roles"] >= min_roles)].head(top_k)

    st.subheader("Top companies (by your warm‑intro potential)")
    st.dataframe(filtered[["display_company", "contacts", "roles", "senior_ratio", "score"]]
                 .rename(columns={"display_company": "Company", "roles": "Open roles", "senior_ratio": "% senior titles"}))

    st.subheader("Company details & roles")
    for _, row in filtered.iterrows():
        comp = row["display_company"]
        with st.expander(f"{comp} — {int(row['contacts'])} contacts • {int(row['roles'])} roles • score {row['score']:.2f}"):
            sample_contacts = (connections_df[connections_df["Company"].str.lower() == comp.lower()]
                               .get("Full Name", pd.Series(["(names unavailable in CSV)"]))
                               .dropna().astype(str).unique().tolist())
            st.write("**Your contacts**:", ", ".join(sample_contacts[:25]) + ("…" if len(sample_contacts) > 25 else ""))
    ...
    score_csv = score_df.to_csv(index=False)
    st.download_button("⬇️ Download jobs_report.csv", data=jobs_csv, file_name="jobs_report.csv")
    st.download_button("⬇️ Download company_scores.csv", data=score_csv, file_name="company_scores.csv")
else:
